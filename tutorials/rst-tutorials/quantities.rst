...
Next, we'll first create a synthetic dataset of radial velocity
...
.. parsed-literal::
    ...
    <matplotlib.text.Text at 0x7fa24489e400>
...
Let's assume that we've mapped the inner part of a molecular cloud in
...
each pixel, so let's calculate that now.
...
We're interested in the integrated intensity over all of the velocity
...
The units of :math:`B` are Hz sec, which can be decomposed to a
...
Suppose you are working on an instrument, and the person funding it asks
...
You meant the inputs to be in arcsec, but alas, you send that to your
